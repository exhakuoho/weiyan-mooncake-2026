...
- **沒有「事先」客製**：月餅要壓的名字／圖案是活動當天現場決定，報名表單刻意不收，
  ...
  模具則是活動後可以帶回家（費用已包含）。
- **Google 端的欄位順序是寫死的**：Apps Script 用 `appendRow` 依序塞 19 欄，對應試算表第 3 列。
  要加／改報名欄位，`route.ts` 的 payload、Apps Script 的 `appendRow`、試算表欄位列
  三者必須同時改，只改一邊會整欄錯位。**新欄位一定要加在最後一欄**，插在中間會讓
  既有報名資料整欄錯開。詳見 README「報名資料流（Google 端）」。
- **報名表單收身分證字號（2026-09-03 起）**：欄位 `participantIds`，投保用，必填，
  是試算表第 19 欄。多人報名時用頓號分隔、依姓名順序填，`route.ts` 的 `splitIds()`
  會正規化（**全形轉半形**、轉大寫、去掉空白與符號、容忍逗號與斜線）。
  **全形是真的會發生**：注音輸入法打出來的「Ａ」看起來跟半形一樣但字元不同，
  2026-09-04 就是因為這個把人擋在門外，`toHalfWidth()` 是為此加的，手機欄也走同一套。
  同理，手機欄刻意**沒有** HTML `pattern`——瀏覽器原生的「請符合所要求的格式」
  會彈在使用者看不懂的地方，改由後端驗並回報是哪一欄。驗證分兩種：本國證號連檢查碼一起驗，
  舊式居留證號（2 個英文字母 ＋ 8 碼）只驗格式。若真的有家長被誤擋，
  放寬的方式是讓 `isNationalId()` 只驗格式、不驗檢查碼，不要整個欄位改成選填——
  保單填不出來反而更麻煩。同意條款那句話已寫明會用於投保，改欄位時不要把它改掉。
